...
document.addEventListener('DOMContentLoaded', () => {
  ...
  let nameText = '';
  let nameIndex = 0;
  // let isNameDeleting = false; // DELETED THIS LINE
  let nameCursorVisible = true;
  let nameCursorInterval; // ADDED: Variable to hold the cursor interval ID

  function typeWriterName() {
    if (nameIndex < name.length) {
      // Logic for typing forward (simplified)
      nameText = name.slice(0, nameIndex + 1);
      nameIndex++;
    } else {
      // Typing is complete
      profileName.textContent = nameText; // Set final text without cursor
      if (nameCursorInterval) {
        clearInterval(nameCursorInterval); // STOP THE BLINKING CURSOR
      }
      return; // Stop the recursive calls
    }

    // Common update logic (show text with cursor while typing)
    ...
    // Schedule the next typing step
    setTimeout(typeWriterName, 300); // Fixed delay for typing
  }

  // UPDATED: Assign the interval ID to the new variable
  ...
  transparencySlider.addEventListener('input', () => {
    ...
    if (opacity == 0) {
      ...
      profileBlock.style.backdropFilter = `blur(${6 * opacity}px)`; /* Adjusted blur factor */
  ...
  // UPDATED handleTilt function for better tilt sensitivity
  function handleTilt(e, element) {
    ...
    // Adjusted maxTilt and division for more controlled sensitivity
    const maxTilt = 10;
